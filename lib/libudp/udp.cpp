...
esp_err_t udp_socket_init(udp_socket_t* object, uint16_t source_port, const char* dest_addr, uint16_t dest_port) {
    if (object == nullptr || dest_port == 0 || dest_addr == nullptr) {
    ...
    if (strcmp(dest_addr, "255.255.255.255") == 0) {
        udp_socket_set_broadcast(object, true);
    }
    return ESP_OK;
}

esp_err_t udp_socket_free(udp_socket_t* object) {
    if (object == nullptr || !object->initialized) return ESP_FAIL;
    close(object->sock);
    object->initialized = false;
    object->sock = -1;
    return ESP_OK;
...
esp_err_t udp_socket_bind_local_port(udp_socket_t* object, uint16_t source_port) {
    ...
    int ret = bind(object->sock, reinterpret_cast<struct sockaddr*>(&object->source_addr), sizeof(struct sockaddr));
    if (ret != 0) {
        ESP_LOGE(TAG, "socket bind failed: errno %d", errno);
        return ESP_FAIL;
    }
...
esp_err_t udp_socket_set_timeout(udp_socket_t* object, uint32_t timeout_s) {
    ...
    ESP_LOGI(TAG, "set timeout to %" PRIu32 " seconds", timeout_s);
    return ESP_OK;
}

esp_err_t udp_socket_send(udp_socket_t* object, uint8_t* data_ptr, size_t size) {
    if (object == nullptr || !object->initialized) return ESP_FAIL;
    ssize_t ret = sendto(object->sock, data_ptr, size, 0,
                         reinterpret_cast<struct sockaddr*>(&object->dest_addr),
                         sizeof(struct sockaddr));
    if (ret < 0) {
        ESP_LOGE(TAG, "sendto failed: errno %d", errno);
        return ESP_ERR_NO_MEM;
    }
    ESP_LOGD(TAG, "sent %zd bytes", ret);
    return ESP_OK;
}
...
esp_err_t udp_socket_recv(udp_socket_t* object, uint8_t* data_ptr, size_t size, size_t* len) {
    ...
    ssize_t recv_len = recvfrom(object->sock, data_ptr, size, 0,
                                reinterpret_cast<struct sockaddr*>(&object->reply_addr), &socklen);
    *len = static_cast<size_t>(recv_len);
    if (recv_len < 0) {
        ESP_LOGE(TAG, "recvfrom failed: errno %d", errno);
        return ESP_FAIL;
    }
    ESP_LOGD(TAG, "received %zd bytes", recv_len);
    return ESP_OK;
}
